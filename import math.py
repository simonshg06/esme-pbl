...
class TicTacToe:
   ...
   def check_winner(self, board, player):
       for i in range(3):
           ...
           if all([board[j][i] == player for j in range(3)]): return True  # Check columns
       if board[0][0] == board[1][1] == board[2][2] == player: return True  # Check main diagonal
       if board[0][2] == board[1][1] == board[2][0] == player: return True  # Check anti-diagonal
       return False


   def is_full(self, board):
       # Returns True if no empty cell remains on the board
   ...
   def minimax(self, board, depth, is_maximizing, ai_piece, collect_scores=None):
       ...
       if self.is_full(board) or depth == 0:
           ...
           if collect_scores is not None:
               collect_scores.append(score)  # Collects leaf scores for display
           return score
           ...
           for r in range(3):
               for c in range(3):
                   if board[r][c] == ' ':
                       ...
                       board[r][c] = ' '  # Undo the move after evaluating
                       best = max(best, val)
           return best
       else:
           best = math.inf
   ...
   def get_all_scores(self, board, ai_piece):
       # Runs minimax and collects all leaf node scores of the current generation
       scores = []
   ...
   def get_ideal_path(self, board, ai_piece, maximizing):
       # Recursively finds the ideal sequence of moves for either the AI or the opponent
       opponent = 'X' if ai_piece == 'O' else 'O'
       ...
       if self.check_winner(board, ai_piece) or self.check_winner(board, opponent) or self.is_full(board):
           return []  # End of the game, stop recursion
       ...
       r, c = best_move
       board[r][c] = current_piece
       rest = self.get_ideal_path(board, ai_piece, not maximizing)  # Continues path alternating players
       board[r][c] = ' '
       return [best_move] + rest  # Builds the path move by move
...
class TicTacToeApp:
   ...
   def _build_ui(self):
       ...
       for text, val in [("Easy (1)", 1), ("Medium (3)", 3), ("Hard (9)", 9)]:
           tk.Radiobutton(diff_frame, text=text, variable=self.depth_var, value=val,
                          command=self._update_depth).pack(side="left", padx=10)
       ...
       for r in range(3):
           for c in range(3):
               btn = tk.Button(board_frame, text="", font=("Arial", 28, "bold"), width=4, height=2,
                               bg="white", command=lambda row=r, col=c: self._on_click(row, col))
   ...
   def _on_click(self, r, c):
       ...
       # In PVC mode, only X (human) can click
